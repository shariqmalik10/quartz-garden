/**
 * The doodle engine for /model-sketchbooks/kimi.
 *
 * Everything a visitor sees on top of the photographs is drawn here, in the
 * browser, with the Canvas 2D API: hand-trembled strokes, lightning, paper
 * boats, butterflies, smoke rings, and little handwritten labels. Marks are
 * generated from a seed with a small mulberry32 PRNG, so every picture has a
 * stable default doodle and "doodle again" is just a new seed.
 *
 * Two things this second sketchbook does that the first did not:
 *   - dashed paths (flight loops, footprints, orbits) are a first-class mark,
 *     so they draw in progressively like every other stroke, and
 *   - once a scene finishes drawing, its sparkles stay awake with a slow,
 *     gentle twinkle — paused off-screen, off-tab, and for reduced motion.
 *
