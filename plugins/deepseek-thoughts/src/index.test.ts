...
describe("DeepSeekThoughts", () => {
  test("only renders the sketchbook on its own route", () => {
    ...
    const vnode = component({ fileData: { slug: "model-sketchbooks/deepseek" } }) as {
      type: string
      props: Record<string, unknown>
    }
    assert.ok(vnode, "expected the sketchbook to render on /model-sketchbooks/deepseek")
  ...
  test("gives every picture a way into the focus view", () => {
    const component = DeepSeekThoughts()
    const vnode = component({ fileData: { slug: "model-sketchbooks/deepseek" } })
